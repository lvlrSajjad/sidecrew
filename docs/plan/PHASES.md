...
| **14c′** | **A big change is not a big file** — inserted by 14c's frozen rule | ⬜ **exit check FROZEN 23 Sep** (`prompts/phase-14c-prime.md`) — one declaration per task, both arms, under ADR-0086 §6 B (built); the same 27 files on a pinned clone · runs when the owner says · `v0.2.0` held |
