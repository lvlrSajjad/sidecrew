...
2. **One gate per workload, and each is an iff in `src/schemas.ts` rather than a convention.**
   ...
   under test **that is neither the whole-body removal nor killed merely by making it throw** (ADR-0089:
   a second mutation pass with the test's assertions stripped identifies crash kills) ∧ non-tautological. Workload #2a (behaviour-preserving changes): survive ⇔ the diff was
