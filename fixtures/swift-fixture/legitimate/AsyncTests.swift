// shape: async — the ordering guarantee, which is the whole point of mapSeries. framework: swift-testing
import Testing
...
@Suite struct MapSeriesAsyncTests {
    ...
    @Test func returnsNothingForAnEmptyList() async throws {
        let empty = await Async.mapSeries([Int]()) { n, _ in n }
        #expect(empty == [])
    }
